...
interface Row {
  moduleLabel: string;
  rowLabel: string; // "View" or the action label
  perm: Permission;
}
...
interface Props {
  open: boolean;
  onClose: () => void;
  /** Optional plane filter — when set, only roles in this plane are shown. */
  plane?: ControlPlane;
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl max-w-[95vw] w-full max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <div>
            <h2 className="text-h3 text-v-charcoal">Role Permission Matrix</h2>
                    ...
                    {roles.map(r => {
                      ...
                      return (
                        <td key={r.name} className="px-2 py-1.5 text-center border border-gray-200">
                          {has ? <Check size={14} className="inline text-v-violet" strokeWidth={3} /> : ''}
                        </td>
                      );
                    })}
                  </tr>
...
function buildWordHtml(roles: typeof ROLE_PERMISSION_MATRIX, rows: Row[]): string {
  const tick = '✓';
  // A4 landscape minus 1cm margins = 27.7cm usable.
  // Function and Capability keep their natural widths; role columns share the rest equally.
  const FUNCTION_CM = 3.5;
  const CAPABILITY_CM = 2.8;
  const ROLE_COL_CM = ((27.7 - FUNCTION_CM - CAPABILITY_CM) / roles.length).toFixed(2);

  // mso-width-source:userset tells Word to respect the declared widths instead of
  // auto-sizing to longest content.
  const colgroup = `
    <colgroup>
  ...
  const headerCells = roles.map(r =>
    `<th style="background:#f3f4f6;border:1px solid #d1d5db;padding:2px 3px;font-size:7pt;vertical-align:bottom;line-height:1.1;text-align:center;white-space:normal;word-wrap:break-word;overflow-wrap:break-word;width:${ROLE_COL_CM}cm;">${escape(r.name)}</th>`
  ).join('');
  ...
  const bodyRows = rows.map((row) => {
    const showModule = row.moduleLabel !== lastModule;
    lastModule = row.moduleLabel;
    const moduleCell = `<td style="border:1px solid #d1d5db;padding:2px 4px;font-weight:${showModule ? 'bold' : 'normal'};font-size:8pt;line-height:1.15;width:${FUNCTION_CM}cm;">${showModule ? escape(row.moduleLabel) : ''}</td>`;
    const capCell    = `<td style="border:1px solid #d1d5db;padding:2px 4px;font-size:8pt;color:#374151;line-height:1.15;width:${CAPABILITY_CM}cm;">${escape(row.rowLabel)}</td>`;
    const roleCells  = roles.map(r => {
      const has = r.permissions.includes(row.perm);
      return `<td style="border:1px solid #d1d5db;padding:2px;text-align:center;font-size:9pt;color:#7c3aed;line-height:1;width:${ROLE_COL_CM}cm;">${has ? tick : ''}</td>`;
    }).join('');
...
<head>
  ...
  <![endif]-->
  <style>
    /* Named section — Word honours the section-level orientation flag on import. */
    @page Section1 {
      size: 29.7cm 21cm;
