// One-off Android OTA release publisher. Mirrors POST /api/admin/mobile-releases.
// Runs INSIDE the vairiot_api container (has minio + prisma + env).
//   node upload-mobile-release.cjs <apkPath> <versionCode> <versionName> [releaseNotes]
const fs = require('fs');
...
(async () => {
  ...
  if (!apkPath || !Number.isInteger(versionCode) || !versionName) {
    throw new Error('usage: node upload-mobile-release.cjs <apkPath> <versionCode> <versionName> [releaseNotes]');
  }
  ...
  await pruneOldMobileReleases().catch(() => {});
