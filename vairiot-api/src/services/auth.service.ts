...
export async function login({ email, password, tenantId }: LoginInput): Promise<AuthTokens> {
  ...
  const accessToken  = signAccessToken({ sub: user.id, tenantId: user.tenantId, email: user.email, roles });
...
export async function refreshTokens(token: string): Promise<AuthTokens> {
  ...
  const roles = user.roles.map((ur) => ur.role.name);
  return {
    accessToken:  signAccessToken({ sub: user.id, tenantId: user.tenantId, email: user.email, roles }),
